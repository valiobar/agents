...
| MongoDB | database | Implemented | Agent-owned `agents` and `conversations` collections |
| Business Service | internal HTTP | Implemented | Company validation and Accountant tools for companies, partners, invoices, expenses, and financial summaries |
...
Routes do not access MongoDB, providers, or service clients directly. Repositories do not contain business rules. Runtime code does not know about FastAPI request objects or HTTP responses. Business-domain access goes through `BusinessClient`.
...
It also creates one long-lived `httpx.AsyncClient` for Business Service during lifespan startup and closes it during shutdown. Business Service creates the indexes for companies, partners, invoices, expenses, and counters.
