...
class AccountantAgent(BaseAgent):
    def get_system_prompt(self) -> str:
        ...
        return (
            ...
            "confirms. For sales invoices that mention product stock, SKUs, inventory items, "
            "or warehouse quantities, do not call create_invoice directly as the first step; "
            "tell the user the reviewed sales invoice inventory workflow can be started so "
            "inventory items and locations are confirmed before invoice creation. Direct "
            "create_invoice remains appropriate for service-only invoices or invoices without "
            "stock-linked lines. Only the frontend can explicitly start that workflow; "
            "do not claim the workflow started automatically. "
            "Before using record_expense, summarize the record and ask for confirmation. "
            f"{company_scope} "
