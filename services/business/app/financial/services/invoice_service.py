...
_ISSUE_STOCK_STATUSES: set[str] = {"sent", "paid", "overdue"}
...
class InvoiceService:
    ...
    async def create_invoice(self, user_id: str, payload: InvoiceCreate) -> InvoiceInDB:
        ...
        if self.inventory_service:
            for item in payload.items:
                if item.inventory_item_id:
                    await self.inventory_service.require_item_for_company(
                        user_id=user_id,
        ...
        if self.inventory_service and created.status in _ISSUE_STOCK_STATUSES and created.company_id:
            await self.inventory_service.issue_for_invoice(
                user_id=user_id,
                invoice_id=created.id,
                company_id=created.company_id,
                lines=[item.model_dump(mode="python") for item in created.items],
            )
        return created
    ...
    async def update_invoice(self, user_id: str, invoice_id: str, payload: InvoiceUpdate) -> InvoiceInDB:
        ...
        should_issue_stock = current.status == "draft" and payload.status == "sent"
        ...
        if updated is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=_INVOICE_NOT_FOUND_DETAIL)
        if self.inventory_service and should_issue_stock and updated.company_id:
            await self.inventory_service.issue_for_invoice(
                user_id=user_id,
                invoice_id=updated.id,
                company_id=updated.company_id,
                lines=[item.model_dump(mode="python") for item in updated.items],
            )
        return updated
