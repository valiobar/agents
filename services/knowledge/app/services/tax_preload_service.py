    ...
    async def preload(self) -> None:
        ...
        for path in sorted(paths):
            try:
                ...
                if not chunks:
                    logger.info("Tax preload: no extractable text", extra={"path": str(path)})
                    continue

                relative_path = str(path.relative_to(root))
                document_id = f"tax:{relative_path}:{content_hash[:12]}"
                ...
                logger.exception("Tax preload: failed to load", extra={"path": str(path)})
                continue
